...
public class AnkiResourceTest {
    ...
    private void createSyntheticApkg() throws Exception {
        ...
        try (Connection conn = DriverManager.getConnection(url);
            ...
            stmt.execute(
                    "INSERT INTO notes (id, guid, flds, mid) VALUES (10, 'guid1', 'Front" + (char) 31 + "Back', 1)");
    ...
    public void testUploadAndPersistence() {
        given()
                ...
                .body("[1].name", is("Test Deck Upload"));

        // Verify CRUD endpoints now have the data
        given()
                .when().get("/v1/decks/100")
                .then()
                .statusCode(200)
                .body("name", is("Test Deck Upload"));

        given()
                .when().get("/v1/models/1")
                .then()
                .statusCode(200)
                .body("name", is("Basic"));

        given()
                .when().get("/v1/notes/10")
                .then()
                .statusCode(200)
                .body("flds", is("Front\u001fBack"));

        given()
                .when().get("/v1/cards/1000")
                .then()
                .statusCode(200)
                .body("ord", is(0));
    }
